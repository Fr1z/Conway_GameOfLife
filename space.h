...
public:
	...
	*/
    ...
    bool checkpoint(long x,long y){ //check if a point exist in space
    ...
    void step(cella &c){
        ...
        if (c.value < 0){ //SE la celle è viva 

            cella* adiacente;

            for (int i=0; i<8 ;i++){ //aggiorna il numero di vicini

                adiacente = (getcella(c.adiacenti[i][0],c.adiacenti[i][1]));
                
                if (adiacente->value<0) { //se è viva
                    c.vicini++;
                }else{//se è una cella virtuale  (before was: else if (adiacente->value<=2) )
                    adiacente->value++;
                }
    ...
    void phase(){ 
       ...
       // std::cout << "fine fase step() - ultima cella su " << celle.size() << std::endl;
        ...
        for (std::list<cella*>::iterator it=celle.begin(); it != celle.end(); ++it){
            ...
            //std::cout<<"-inizio giudizio cella:  " << tmp.x << ", " << tmp.y << std::endl;
            
            if ( ( (tmp.value < 3) && (tmp.value != -1) ) || tmp.vicini>3 ) {  //morte per insufficienza o sovrappopolamento
              //  std::cout<<"la cella muore per insufficienza value: " << tmp.value <<  std::endl;
                delete *it;
            }else if (tmp.value == 3){ //nascita di una nuova cella 
                (**it).value = -1;
                (**it).vicini = 0;
                newcelle.push_back(*it);
				//std::cout<<"nascita di una nuova cella " << tmp.value << " " << (**it).value <<std::endl;
            }else if (tmp.value = -1){// conservazione o dispersione di una cella VIVA
                //morte per dispersione
                if (tmp.vicini < 2){
                    //std::cout<< "la cella muore per dispersione: " << tmp.vicini <<  std::endl;
                    delete *it;
                }else{
                    //persistenza
                    //std::cout<<"la cella persiste +vita, reset vicini " << std::endl;
                    (**it).vicini = 0;
                    (**it).vita++;
                    newcelle.push_back(*it);
                }
            ...
            //std::cout<<"-fine giudizio:  " << (*it).x << ", " << (*it).y << std::endl;
        }

        celle.clear();
        celle.swap(newcelle);

        //std::cout<<"fine fase, generazione  " << std::endl;
        generazione++;
		/*
        std::cout<<"celle sopravvissute: " << std::endl;
        for (std::list<cella*>::iterator it=celle.begin(); it != celle.end(); ++it)
            std::cout<< "(" << (**it).x << "," << (**it).y << "), " ;
