	...
	int handle(int event){
		...
		switch(event){
			...
			case FL_KEYDOWN:  //if event is key down
				switch(Fl::event_key()) {
					case FL_Left: cx += -1; break;  	// left arrow
					case FL_Right: cx += 1; break;  	// left arrow
					case FL_Up:	cy += -1; break;		// up arrow
					case FL_Down: cy += 1; break;		// down arrow
				}
				this->redraw();
				return 1;
			...
			case FL_PUSH:
				//std::cout << "coord: " << Fl::event_x() << "," << Fl::event_y() << std::endl;
				//std::cout << "vcoord: " << get_virtual_pos().x << "," << get_virtual_pos().y << std::endl;
				//std::cout << "rpos: " <<get_real_pos(get_virtual_pos().x,get_virtual_pos().y).x << std::endl;
			...
			case FL_DRAG:
				if (editing){
					...
					if (drag_start.x == -1){ //if drag start
						drag_start.x = Fl::event_x();
						drag_start.y = Fl::event_y();
					}else{
						if ((cx >= x_min) && (cy >= y_min) && ( (cx+(_w/(scale*c_dim))) <= x_max) && ( (cy+(_h/(scale*c_dim))) <= y_max) ){ //limit shift out	
			...
			case FL_MOUSEWHEEL:
				...
				if (Fl::event_dy() == 1){
					//ZOOM OUT
					if ( ( (cx+(_w/(scale*c_dim))) < x_max) && ( (cy+(_h/(scale*c_dim))) < y_max) ){ //limit zoom-out of universe
						//new visual relative to zoom out point	
						cx = ( get_virtual_pos().x - round((_w/((scale-0.09)*c_dim))*kw)
						- round( 2* (_w/c_dim)*(1/(scale-0.09)-(1/scale)) )*(0.5-kw) );
						cy = ( get_virtual_pos().y - round((_h/((scale-0.09)*c_dim))*kh)
						- round( 2* (_h/c_dim)*(1/(scale-0.09)-(1/scale)) )*(0.5-kh) );
			...
			default:
				//Usually  FL_NO_EVENT
				//std::cout << "Event: " << fl_eventnames[event] << std::endl;
				return 0;
